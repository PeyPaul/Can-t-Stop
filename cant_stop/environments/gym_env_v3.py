...
class CantStopGymEnv(gym.Env):
    ...
    def step(self, action): 
        ...
        if self.should_continue_RL(self, action):
            ...
            # self.reward += 1
            # print("RL décide de continuer, reward +1")
    ...
    def get_observation(self, possible, done=False, temp_markers={}): 
        ...
        num_temp_markers = np.array([3 - len(self.temp_markers)], dtype=np.int32)  # 1 valeur
        ...
        for idx in range(len(COLUMNS)): #on update les marqueurs temporaires
            if idx+2 in self.temp_markers.keys():
                temp_markers_progress[idx] = COL_LENGTHS[idx+2] - self.temp_markers[idx+2]
            else :
                temp_markers_progress[idx] = COL_LENGTHS[idx+2] - self.players[0].progress[idx+2]
    ...
    def play_turn_RL(self, game_state, player, action):
        ...
        for val in choice:
            ...
            self.reward += 1
            # print(f"RL avance le marqueur temporaire sur la colonne {val}, reward +1")
